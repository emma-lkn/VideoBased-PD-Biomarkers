...
class keypointext:
    ...
    def preprocess_and_display_video(self, video_path, label, participant_id): # added participant_id
        """Process video and display frames with calculated distances or angles."""
        # hand_to_track = 'Right' if 'R' in video_path else 'Left' if 'L' in video_path else None #TODO change for correct naming style
        ...
        participant_folder = os.path.join(self.config['save_path'], str(participant_id).zfill(3)) # added this so videos are stored in subfolders
        os.makedirs(participant_folder, exist_ok=True)
        save_vid_path = os.path.join(participant_folder, self.new_filename) # changed self.config['save_path'] to participant_folder
        ...
        while cap.isOpened():
            ...
            frame_timestamp_ms = int((total_frames / self.fps) * 1000)
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            output.write(annotated_frame_bgr)
    ...
    def run_keypoints(self):
         ...
         video_labels = pd.read_csv(self.config['sat_ft']) # changed vid2score to sat_ft
         print(video_labels.columns)
         ...
         skipped_videos = [] # to store videos that might crash the processing

         try: # to ensure pkl file is saved even in case of it crashing
            for vid, idi, label in tqdm(zip(videos, ids, labels), total=len(videos), desc="Processing videos"):
                try: # so it doesnt crash in case of a wrongly processed video
                    self.vid = vid
                    result = self.preprocess_and_display_video(vid, label, idi) # changed var name to result to check for None
                    
                    if result is None:
                        skipped_videos.append(vid)
                        continue
        
                    distance, p_keypoint = result # added this
                    ...
                    final_data = {
                        ...
                        'id':all_ids,
                        'fps':all_fps,
                        'skipped_videos': skipped_videos # added this
                    }
                    ...
                    cv2.destroyAllWindows() # in case OpenCV windows are still open after exception
                    skipped_videos.append(vid)
                    continue
          
         finally: # final save even if it crashes
            final_data = {
            'video_path': all_video_paths,
            'distances': all_distances,
            'keypoints': all_keypoints,
...
if __name__ == "__main__":
    ...
    print("csv path:", os.path.join(project_root, "data/raw/sat_finger_tapping.csv"))
    print("exists:", os.path.exists(os.path.join(project_root, "data/raw/sat_finger_tapping.csv"))) 

    CONFIG = {
        "sat_ft": os.path.join(project_root, "data/raw/sat_finger_tapping.csv"), # changed file name
